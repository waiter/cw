...
bool Button::init(CCSpriteFrame* spriteframe , CCObject* rec, SEL_MenuHandler selector)
{
	...
	do 
	{
		...
		p_actionRect = CCRectMake(0,0,s.width,s.height);
...
{
	//CCSize s = getContentSize();
	return CCRectMake(-m_obAnchorPointInPoints.x + p_actionRect.origin.x, -m_obAnchorPointInPoints.y + p_actionRect.origin.y , p_actionRect.size.width , p_actionRect.size.height);
}
